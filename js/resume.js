...
const loadResume = () => {
	...
	if (resumeData.length === 0) {
		...
		if (resumeStartIndex < resumeData.length) {
			...
			positionAppliedFor.innerText =
				positionTitle + resumeData[resumeStartIndex].basics.AppliedFor;
			...
			techSkills.innerHTML = `<div>${skills
				.map((keyword) => `<p>${keyword}</p>`)
				.join("")}</div>`;
			hobbiesInfo.innerHTML = `<div>${hobbies
				.map((hobby) => `<p>${hobby}</p>`)
				.join("")}</div>`;
			...
			experiences.innerHTML = `<div>${Object.keys(
				resumeData[resumeStartIndex].work
			).map(
				(key) =>
					`<p class='experience'><b>${key}</b>: ${resumeData[resumeStartIndex].work[key]}</p>`
			)}</div>`.replaceAll(",", "");

			education.innerHTML = `<ul>${Object.keys(
				resumeData[resumeStartIndex]["education"]
			)
				.map(
					(education) =>
			...
			internship.innerHTML = `<ul>${Object.keys(
				resumeData[resumeStartIndex]["Internship"]
			)
				.map(
					(key) =>
						`<li><b>${key}</b>: ${resumeData[resumeStartIndex]["Internship"][key]}</li>`
				)
				.join("")}</ul>`;
			achievement.innerHTML = `<ul>${resumeData[resumeStartIndex][
				"achievements"
			]["Summary"]
